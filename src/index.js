...
const getMappedCountries = (countries_stat, countries) => {
  ...
  countries_stat.forEach(country => {
    const cf = countries.find(
      co => co.name.toLowerCase() === country.country_name.toLowerCase()
    );
    if (cf) {
...
const loadCovidData = callback => {
  return db
    ...
    .then(snap => {
      ...
      document.getElementById("updated").innerHTML = `Last Update: ${new Date(
        date
      ).toLocaleDateString()}`;
